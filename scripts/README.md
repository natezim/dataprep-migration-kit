...
build the catalog + backlog. **READ-ONLY** — these only `GET` from Dataprep; they never modify a
...
- A local `.env` (copy `.env.example`) with `DATAPREP_API_BASE_URL` and `DATAPREP_API_TOKEN`.
  (Optional `GCP_PROJECT` for the catalog.)
- Python 3.10+. Scripts 01–03 are stdlib-only. Script 04's Excel output needs
  ...
  `catalog.json` (the source of truth).
...
python 02_download_unzip.py   # -> context/<plan>/<flow>/  (exported packages)
python 03_job_stats.py        # -> output/temp/job_stats.json  (runs, durations, owners)
python 04_compile_catalog.py  # -> docs/catalog.json, output/backlog.md, (output/data/catalog.xlsx)
```

Then in Gemini CLI, run `/dp:start` — it reads the catalog/backlog and helps you migrate one
flow at a time.
